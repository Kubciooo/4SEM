...
def generate_codebook(data, size_codebook, epsilon = 0.0001):
...
def split_codebook(data, codebook, epsilon, initial_average_distance):
    ...
    for code in codebook: 
        code1 = create_new_codevector(code,0.1) 
        code2 = create_new_codevector(code, -0.1)
    ...
    num_iter  = 0
    while error > epsilon:
        print(f"iteracja numer {num_iter}")
...
def create_new_codevector(code_vector, moving_factor):
    return [int(x * (1.0+moving_factor)) for x in code_vector]
...
def average_vector_of_vectors(vectors, dim=None, size = None):
    ...
    for i in range(d):
        average_vector[i] /= s 
        average_vector[i] = int(average_vector[i])
