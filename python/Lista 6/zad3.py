...
class NeuralNetwork:
    def __init__(self, x, y, func, func_derivative, eta = 0.001):
        ...
        self.weights2           = np.random.rand(10,10)*2 - 1.
        self.weights3           = np.random.rand(1,10)*2 - 1.
    ...
    def feedforward(self):
        self.layer1 = self.func(np.dot(self.input, self.weights1.T)+self.bias1)
        self.layer2 = self.func(np.dot(self.layer1, self.weights2.T)+self.bias2)
        self.output = self.func(np.dot(self.layer2, self.weights3.T))


    def backprop(self):
        delta3 = (self.y - self.output) * self.func_derivative(self.output)
        d_weights3 = self.eta * np.dot(delta3.T, self.layer2)

        delta2 =  self.func_derivative(self.layer2) * np.dot(delta3, self.weights3)
        ...
        for d in delta3:
            self.bias2 -= d*self.eta
        for d in delta2:
            self.bias1 -= d*self.eta


    def predict(self, predict_x):
        self.layer1 = self.func(np.dot(predict_x, self.weights1.T)+self.bias1)
        self.layer2 = self.func(np.dot(self.layer1, self.weights2.T)+self.bias2)
        self.output = self.func(np.dot(self.layer2, self.weights3.T)) 
        return self.output
...
def wykonaj_iteracje(siec1,dane, count = 500000):
...
a = np.min(y)
b = np.max(y)
siec1 = NeuralNetwork(x,y,ReLu,ReLu_derivative,eta=0.00000001)

if a >= 0 and b <= 1:
    siec1 = NeuralNetwork(x,y,sigmoid,sigmoid_derivative,eta=0.001)
elif a >= -1 and b <=1:
